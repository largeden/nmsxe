...
var severity_sheet = {
    ...
    select : function() {
        ...
        if(severity_sheet.colorset == 'white') $(this).css('background','#eaeaea');
    ...
    ready : function() {
        $('body').ready(function() {
            // select 형식의 표현을 바꿈
            if(!$.browser.msie) if($('select.select').is('.select')) severity_sheet.select();
            ...
            $('select.severity_aware').change(function() {
                severity_sheet.doSeverityAware($(this).val());
            });
        });
    }
};
